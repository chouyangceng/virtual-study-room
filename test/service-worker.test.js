...
test('activation removes only stale virtual-study-room caches', async () => {
  const worker = loadServiceWorker({
    cacheKeys: ['virtual-study-room-v20', 'virtual-study-room-v22', 'virtual-study-room-v23', 'mistake-notebook-v1']
  });
  let activation;
  worker.handlers.activate({ waitUntil: promise => { activation = promise; } });
  await activation;
  assert.deepEqual(worker.deleted, ['virtual-study-room-v20', 'virtual-study-room-v22']);
});
